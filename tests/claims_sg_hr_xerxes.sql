...
END;
/
-- Expected: Should be not able to update finance_approved_by
DECLARE
    counter INT;
BEGIN
    UPDATE SYSTEM.claims SET 
    ...
    counter := SQL%rowcount;
    IF counter != 0 THEN
        RAISE_APPLICATION_ERROR(-20000, 'Should not able to update finance_approved_by');
    END IF;
    ROLLBACK;
END;
/
-- Expected: Should be able to update hr_approved_by
DECLARE
    counter INT;
BEGIN
    UPDATE SYSTEM.claims SET 
    ...
    counter := SQL%rowcount;
    IF counter != 1 THEN
        RAISE_APPLICATION_ERROR(-20000, 'Should be able to update hr_approved_by');
    END IF;
    ROLLBACK;
END;
/
